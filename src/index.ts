...
export default function json2TsPlugin(options: Json2TsPluginOptions): Plugin {
  ...
  const generate = () => {
    ...
    var result = `export${
      isDefaultImport ? " default" : ""
    } class ${className}`;
    var getSpaces = function (depth: number) {
      var result = "\n";
      while (depth > 0) {
        result += "    ";
        depth--;
    ...
    var escapeHtml = function (val: string): string {
      return val
        .replace(/&/g, "&amp;")
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")
    ...
    var iteratee = function <T extends { [key in keyof T]: unknown }>(
      obj: T,
      result: string,
      objPath: string,
      depth: number
      ...
      for (var objKey in obj) {
        ...
        if (obj[objKey] != null && typeof objValue === "object") {
          if (objPath) {
            ...
            result += iteratee(
              objValue,
              `${spaces}public static ${objKey} = `,
              objPath + objKey + ".",
              depth + 1
            );
          }
        } else if (objPath) {
          result += `${spaces}/** ${escapeHtml(String(objValue))} */${getSpaces(
            depth
          )}${objKey}: "${objPath + objKey}",`;
        } else {
          result += `${spaces}/** ${escapeHtml(String(objValue))} */${getSpaces(
            depth
          )}${objKey} = "${objPath + objKey}";`;
